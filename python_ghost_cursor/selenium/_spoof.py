...
class GhostCursor:
    def __init__(self, driver: WebDriver, start: Vector):
        self.page = driver
        self.previous = start
        self.moving = False
    ...
    def get_random_page_point(self) -> Vector:
        ...
        window_size = self.page.get_window_size()
        ...
        for v in vectors:
            try:
                ...
                if abort_on_move and self.moving:
                    return
                action_chain = ActionChains(self.page)
                action_chain.w3c_actions.wheel_action.scroll(x=v.x, y=v.y)
                self.previous = v
    ...
    def click(
        ...
        if selector is not None:
            self.move(selector, padding_percentage)
            self.toggle_random_move(False)

        try:
            action_chain = ActionChains(self.page)
            action_chain.w3c_actions.pointer_action.pointer_down()
            if wait_for_click is not None:
                time.sleep(wait_for_click / 1000)
            action_chain.w3c_actions.pointer_action.pointer_up()
            action_chain.w3c_actions.key_action.pause()
            action_chain.w3c_actions.key_action.pause()
        except Exception as exc:
    ...
    def move(
        ...
        ActionChains(self.page).scroll_to_element(elem)
        box = {**elem.size, **elem.location}
        if box is None:
            raise Exception(
